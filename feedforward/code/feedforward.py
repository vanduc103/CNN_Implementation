...
weights = {
  ...
  'hidden3': tf.get_variable('w_hidden3', shape=[hidden_num_units[1], hidden_num_units[2]],
                initializer=tf.contrib.layers.xavier_initializer()),
  'output': tf.get_variable('w_output', shape=[hidden_num_units[2], output_num_units],
                initializer=tf.contrib.layers.xavier_initializer())
}
...
with tf.Session() as sess:
  ...
  for epoch in range(epochs):
    ...
    prediction = tf.equal(tf.argmax(tf.nn.softmax(output_layer), 1), tf.argmax(y, 1))
